...
# ---------------------------------------------------------------------------
# Step 8: pdflatex compile
# ---------------------------------------------------------------------------

def try_compile(tex_path: Path) -> tuple[bool, str]:
    """Run pdflatex twice. Return (ok, last_log_tail)."""
    if shutil.which("pdflatex") is None:
        return False, "pdflatex not on PATH (brew install --cask basictex)"
    log_tail = ""
    for pass_num in (1, 2):
        proc = subprocess.run(
            [
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ok:
        print("pdflatex: OK (paper.pdf built)")
    else:
